...
export function AnnotationLayer({ pageId, content, children }: AnnotationLayerProps) {
  ...
  useEffect(() => {
    ...
    // Wait for content to render (longer delay for markdown processing)
    const timer = setTimeout(() => {
      if (contentRef.current) {
        const rect = contentRef.current.getBoundingClientRect()

        // Get the actual rendered height by finding the bottom of the last child
        let maxHeight = contentRef.current.scrollHeight

        // Alternative: measure from top to bottom of last element
        const children = contentRef.current.children
        if (children.length > 0) {
          const lastChild = children[children.length - 1]
          const lastChildRect = lastChild.getBoundingClientRect()
          const containerRect = contentRef.current.getBoundingClientRect()
          maxHeight = Math.max(maxHeight, lastChildRect.bottom - containerRect.top)
        }

        console.log('[AnnotationLayer] Canvas size:', {
    ...
    }, 500) // Longer delay to ensure markdown is rendered
