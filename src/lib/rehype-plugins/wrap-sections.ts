...
/**
 * Rehype plugin to wrap h1-h3 headings and their content into section elements
 * for the annotation system.
 *
 * This creates a proper DOM structure where each section contains:
 * - The heading element (h1-h3)
    ...
    for (const child of tree.children) {
      if (child.type === 'element' && isHeading(child.tagName)) {
        ...
        if (level >= 1 && level <= 3) {
          ...
          // h4-h6 or other elements - add to current section or top level
          if (currentSection) {
